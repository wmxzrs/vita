var gitblog = function (config) {
    ...
    show: function () {
      ...
      if (Object.keys(config.friends).length != 0) {
        ...
        menu_friend.innerHTML =
          '<li><text style="font-zise:14px"><span style="color: white;transform:translateX(4px)">友链：</span></text></li>';
    ...
    init: function () {
      ...
      if (icon.href != undefined && icon.href != null) {
        ...
        document.getElementById("div_" + icon.name).innerHTML +=
          ...
          '" id="icon_' +
          icon.name +
          '" style="width:50px;margin-left:10px;margin-right:10px;cursor:pointer">';
      }
  ...
  var Footer = function () {
    ...
    this.icon_num = 0;
    this.content =
      'Powered by <a href="https://github.com/imuncle/gitblog" target="_blank" style="color: aquamarine;text-decoration:none;border-bottom: 1px solid #79f8d4;">gitblog</a>';
  };
  ...
  Footer.prototype = {
    showIcon: function () {
      ...
      for (var i in config.icons) {
        if (
          config.icons[i].icon_src != undefined &&
          config.icons[i].icon_src != null
        ) {
          document.getElementById("icon").innerHTML +=
            '<div style="padding-inline-start: 0;margin: 0" id="div_' +
            i +
            '"></div>';
        }
      }
      for (var i in config.icons) {
        if (
          ...
          var left =
            Object.keys(config.icons).length * 35 -
            70 * footer.icon_num +
            config.icons[i].width / 2 -
            35;
  ...
  Pages.prototype = {
    ...
    show: function () {
      ...
      document.getElementById("pages").innerHTML =
        '<li id="last_page"><a id="last" style="cursor: pointer">«</a></li>';
      if (page.pages <= page.pageLimit) {
        ...
        if (page.page >= 5) {
          document.getElementById("pages").innerHTML +=
            '<li><a id="page1" style="cursor:pointer">1</a></li>';
          document.getElementById("pages").innerHTML +=
            '<li><a style="cursor:pointer;pointer-events: none;">...</a></li>';
          document.getElementById("pages").innerHTML +=
            '<li><a id="page' +
            (page.page - 1) +
            '" style="cursor:pointer">' +
      ...
      document.getElementById("pages").innerHTML +=
        '<li id="next_page"><a id="next" style="cursor: pointer">»</a></li>';
      for (var i = 1; i <= page.pages; i++) {
        if (self.options.label != undefined) {
          $("#page" + i).click(function () {
            window.location.href =
              "?label=" + self.options.label + "&page=" + this.innerHTML;
          });
        } else if (self.options.id != undefined) {
          $("#page" + i).click(function () {
            window.location.href =
              "?id=" + self.options.id + "&page=" + this.innerHTML;
          });
        } else if (self.options.q != undefined) {
          $("#page" + i).click(function () {
            window.location.href =
              "?q=" + self.options.q + "&page=" + this.innerHTML;
          });
        } else {
          $("#page" + i).click(function () {
    ...
    last: function () {
      ...
      if (self.options.label != undefined) {
        window.location.href =
          "?label=" + self.options.label + "&page=" + this.page;
    ...
    show: function (type, id) {
      ...
      if (reaction.isLike == false) {
        document.getElementById(id).innerHTML =
          '<img src="images/heart.svg" style="height:20px;float:left">';
  ...
  Comment.prototype = {
    ...
    init: function () {
      ...
      $.ajax({
        ...
        headers: {
          Accept:
            "application/vnd.github.squirrel-girl-preview, application/vnd.github.html+json, application/x-www-form-urlencoded",
        },
        url:
          "https://api.github.com/repos/" +
          config.name +
      ...
      if (comment.login == false) {
        login.innerHTML =
          '<a class="gitment-editor-login-link" id="log">登入</a> with GitHub';
      } else {
        login.innerHTML =
          '<a class="gitment-editor-login-link" id="log">登出</a>';
      }
      ...
      if (comment.login == false) {
        ...
        editor_content.innerHTML =
          '<textarea placeholder="(发表评论)" id="comment-input"></textarea>';
    ...
    checkIsLogin: function () {
      ...
      if (this.login == true) {
        $.ajax({
          ...
          success: function (data) {
            ...
            window.localStorage.setItem(
              "authorize",
              btoa(data.login + ":" + window.localStorage.access_token)
            );
          },
          error: function () {
        ...
        avatar.innerHTML =
          ...
          '<img src="images/gitment-github-icon.svg" class="gitment-github-icon" style="width:44px">' +
          "</a></div>";
    ...
    preview: function () {
      ...
      if (comment_input == "") {
        ...
        $.ajax({
          ...
          headers: {
            Accept:
              "text/html, application/vnd.github.squirrel-girl-preview, application/vnd.github.html+json, application/x-www-form-urlencoded",
    ...
    init: function () {
      ...
      if (self.options.token != undefined && self.options.token != null) {
        ...
        history.replaceState(
          null,
          config.title,
          "content.html?id=" + self.options.id
        );
      }
      article.comment_url =
        "https://api.github.com/repos/" +
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          for (var i in data.labels) {
            labels.innerHTML +=
              '<a href="issue_per_label.html?label=' +
              data.labels[i].name +
              '"># ' +
              data.labels[i].name +
              "</a>";
    ...
    addItem: function (data) {
      ...
      for (var i in data) {
        ...
        for (var j in data[i].labels) {
          labels_content +=
            "<li><a href=issue_per_label.html?label=" +
    ...
    init: function () {
      ...
      if (self.options.label == undefined) {
        if (self.options.q == undefined) {
          this.issue_url =
            "https://api.github.com/repos/" + config.name + "/" + config.repo;
          this.issue_perpage_url =
            "https://api.github.com/repos/" +
            config.name +
            "/" +
    ...
    init: function () {
      ...
      $(".search-input").bind("keypress", function (event) {
        if (event.keyCode == "13" && $(".search-input").val() != "") {
          window.location.href =
            "issue_per_label.html?q=" + $(".search-input").val();
        }
      });
